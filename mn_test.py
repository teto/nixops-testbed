...
topoSymetricJitter = [
    {'bw': 10, 'delay': "20ms", "loss": 0, "jitter": '5ms'},
    {'bw': 10, 'delay': "20ms", "loss": 0, "jitter": '5ms'},
]
...
class IperfTest(Test):
    ...
    def run_xp(self, client, server, run, **kwargs):
        ...
        # number of bytes "-n" or duration "-t"
        if FILE_TO_TRANSFER is not None:
            cmd.append("-F")
            cmd.append(FILE_TO_TRANSFER)
            cmd.append("-n")
            cmd.append(dataAmount)
        else:
            # in seconds
            cmd.append("-t")
...
class IperfNetlink(IperfTest):
    def __init__(self, *args, **kwargs):
        """
        """
        IperfTest.__init__(self, *args, **kwargs)
        self.name = "Iperf_netlink"
        self.description = "iperf test"
